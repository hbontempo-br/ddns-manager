...
class TestGoogleSyntheticDDNSUpdater(unittest.TestCase):
    def test_getter_uses_correct_address(self):
        ...
        def req(method: str, url: str, params: dict):
            expected_url = f"https://{username}:{password}@{GoogleSyntheticDDNSUpdater.base_url}"
            expectd_param = {
                'hostname': hostname,
                'myip': ip
            }
    ...
    def test_get_current_ip_raises_error_if_post_fails(self):
        ...
        du = GoogleSyntheticDDNSUpdater(
            ...
            password='password',
            hostname='hott.name',
            req=lambda method, url, params: ResponseMock(status_code=invalid_status_code, text='good')
        )
    ...
    def test_get_current_ip_raises_error_if_post_return_not_success(self):
        ...
        du = GoogleSyntheticDDNSUpdater(
            ...
            password='password',
            hostname='hott.name',
            req=lambda method, url, params: ResponseMock(status_code=200, text=invalid_response)
        )
    ...
    def test_get_current_ip_not_raises_error_if_post_succeeds(self):
        ...
        du = GoogleSyntheticDDNSUpdater(
            ...
            password='password',
            hostname='hott.name',
            req=lambda method, url, params: ResponseMock(status_code=200, text=valid_response)
        )
